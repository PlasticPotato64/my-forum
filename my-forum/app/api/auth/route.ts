...
export async function GET(req: NextRequest) {
  try {
    ...
    const sql = getDb()
    const results = await sql`SELECT username, avatar FROM accounts WHERE username ILIKE ${'%' + q + '%'} LIMIT 8`
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (username.length > 16) return NextResponse.json({ error: 'Username max 16 characters' }, { status: 400 })
    if (action === 'register') {
