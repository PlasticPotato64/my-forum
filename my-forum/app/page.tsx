...
type Post = { id: string; title: string; body: string; author: string; authorAvatar: string; category: string; createdAt: string; replies: any[] }
  ...
  return (
    <div style={{ display: 'flex', gap: '24px', paddingTop: '32px' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ paddingBottom: '28px', borderBottom: '1px solid var(--border)', marginBottom: '24px' }}>
          ...
          <p style={{ color: 'var(--text2)', fontSize: '13px' }}>{posts.length} threads · join the conversation</p>
        </div>
        {loading ? (
            ...
            {posts.map((post, i) => (
              <div key={post.id} style={{ position: 'relative' }}>
                <Link href={`/forum/${post.id}`}>
                  <div style={{ padding: '18px 20px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '10px', transition: 'all 0.15s', cursor: 'pointer', animation: 'fadeUp 0.3s ease both', animationDelay: `${i * 0.04}s` }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '16px', fontWeight: 600, color: 'var(--text)', marginBottom: '4px', lineHeight: 1.3 }}>{post.title}</h2>
      ...
      <div style={{ width: '220px', flexShrink: 0, paddingTop: '4px' }}>
        <div style={{ position: 'sticky', top: '72px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search username..." style={{ width: '100%', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '8px', padding: '8px 10px', color: 'var(--text)', fontSize: '12px', fontFamily: 'var(--font-mono)' }}
              onFocus={e => e.target.style.borderColor='var(--accent)'} onBlur={e => e.target.style.borderColor='var(--border)'} />
            {search && (
              <div style={{ marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {searching && <p style={{ fontSize: '11px', color: 'var(--text3)', padding: '4px' }}>searching...</p>}
                {!searching && userResults.length === 0 && <p style={{ fontSize: '11px', color: 'var(--text3)', padding: '4px' }}>No users found</p>}
                {userResults.map((u: any) => (
                  <Link key={u.username} href={`/profile/${u.username}`}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 8px', borderRadius: '8px', transition: 'background 0.1s' }}
                      onMouseEnter={e => (e.currentTarget.style.background='var(--bg3)')} onMouseLeave={e => (e.currentTarget.style.background='transparent')}>
                      <Avatar username={u.username} avatar={u.avatar} size={24} linkToProfile={false} />
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px' }}>
            ...
            {[
              ...
              <Link key={l.href} href={l.href} style={{ display: 'block', padding: '6px 4px', fontSize: '12px', color: 'var(--text2)', transition: 'color 0.1s' }}
