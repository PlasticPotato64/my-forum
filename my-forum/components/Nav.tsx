    ...
    <nav style={{ borderBottom: '1px solid var(--border)', background: 'rgba(10,10,15,0.95)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 100 }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 20px', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <Link href="/" style={{ padding: '6px 14px', borderRadius: '6px', fontSize: '13px', color: 'var(--text2)' }}
            onMouseEnter={e => (e.currentTarget.style.background='var(--bg3)')} onMouseLeave={e => (e.currentTarget.style.background='transparent')}>Forums</Link>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowGames(!showGames)} style={{ padding: '6px 14px', borderRadius: '6px', fontSize: '13px', color: 'var(--text2)', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}>Games ▾</button>
            {showGames && (
              <div style={{ position: 'absolute', left: 0, top: '44px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden', minWidth: '160px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                <Link href="/games/rng" onClick={() => setShowGames(false)} style={{ display: 'block', padding: '11px 16px', fontSize: '13px', color: 'var(--text2)' }}
                  onMouseEnter={e => (e.currentTarget.style.background='var(--bg3)')} onMouseLeave={e => (e.currentTarget.style.background='transparent')}>🎲 Title RNG</Link>
              </div>
            )}
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {user ? (
            <>
              ...
              {/* Points badge */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: '20px', padding: '3px 10px 3px 4px' }}>
                <StarIcon />
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap' }}>{formatPoints(points)}</span>
              </div>
              <div style={{ position: 'relative' }}>
                ...
                {showMenu && (
                  <div style={{ position: 'absolute', right: 0, top: '44px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden', minWidth: '180px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                    ...
                    <button onClick={() => { logout(); setShowMenu(false) }} style={{ width: '100%', padding: '10px 16px', background: 'transparent', color: 'var(--red)', fontSize: '13px', fontFamily: 'var(--font-mono)', textAlign: 'left', cursor: 'pointer', border: 'none', borderTop: '1px solid var(--border)' }}>Log out</button>
                  </div>
                )}
              </div>
            </>
